...
\section{Random-weight neural network with \texttt{ReLu} activations}\label{sec:ReLu-RWNN}
...
\begin{align*}
    ...
    %\EE^\Phi\bigg[\Big\| \widehat \Uf_{i+1}(\XX_{t_{i+1}}) - \Big[\Uf_i(\XX_{t_i};\Theta^i) - f\Big(t_{i}, \XX_{t_{i}}, \Uf_i(\XX_{t_i};\Theta^i), \Zf_{i}(\XX_{t_i}; \Theta^i)\Big)\Deli \\
...
We now summarise the algorithmic procedure of our RWNN scheme in Algorithm~\ref{alg:markovian} below. See how the algorithm resembles the Least-Square Monte-Carlo method of~\cite{Longstaff2001ValuingApproach} after considering the sample estimator version of RLS from Section~\ref{sec:randomizedLS}.
...
\newpage
\subsection{Algorithm}
We summarise the steps of the Algorithm~\ref{alg:nonmarkovian}.
