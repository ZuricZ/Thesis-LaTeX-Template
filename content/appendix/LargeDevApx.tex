...
Then, observing that~$\Phi_t$ is continuous and bounded, we have by the Continuous mapping theorem that $\lim_{\eps\downarrow 0} \EE[\Phi_t(V^\eps)]=\EE\left[\Phi\left(\bar\psi\right)\right]$. Next, by definition of~$\Phi_t$, $\bar{\psi}=v_0 + \int_0^\cdot f\left(\bar{\psi}_s\right)\D s$ will be the limiting function if we are able to show the above limit is zero for that particular choice of~$\bar\psi$. Indeed, by H{\"o}lder's inequality % and It{\^o} isometry
\begin{align*}
\EE[\Phi_t(V^\eps)]
